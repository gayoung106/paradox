...
절편동일성이 지지되지 않았으므로, 위 잠재평균 값은 참고용으로만 제시하며 본문 해석에는 사용하지 않는다(Vandenberg & Lance, 2000).
...
동일하게 추가 제약한 모형으로, χ²(770) = 2856.06,
CFI = 0.937, RMSEA = 0.037,
SRMR = 0.043로 나타났다. 요인동일성 모형과 비교한 결과
ΔCFI = -0.012, ΔRMSEA = 0.003,
ΔSRMR = -0.001로, Cheung과 Rensvold(2002)의 기준에 따라
절편동일성이 지지되지 않았다.
