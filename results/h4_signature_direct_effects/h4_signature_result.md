# H4 차별적 행동 서명 검정: DEI 차원별 OCB/UPB 직접효과

- 표본 크기: N = 2020
- 연령 기준: 2024년 기준으로 통일 (age = 2024 - SQ1K2_1)
- 부트스트랩: 5,000회 (케이스 재표집, semopy 매회 재적합), 유효 반복 5,000회
- 잭나이프: 2,020회 (BCa 가속도 보정 a 산출용)

## 1. 4개 직접경로 표준화 β (semopy, BCa 95% CI)

| 직접경로     |   표준화 β | 95% BCa CI      |
|:---------|--------:|:----------------|
| 포용 → OCB |   0.331 | [0.269, 0.392]  |
| 형평 → OCB |  -0.055 | [-0.109, 0.002] |
| 형평 → UPB |   0.216 | [0.151, 0.282]  |
| 포용 → UPB |  -0.032 | [-0.095, 0.030] |

## 2. H4 공식 검정 — 4개 대비

| 대비                           |   점추정치 | 95% BCa CI     | 0 포함여부   |
|:-----------------------------|-------:|:---------------|:---------|
| d_OCB = (포용→OCB) − (형평→OCB)  |  0.386 | [0.281, 0.486] | 미포함      |
| d_UPB = (형평→UPB) − (포용→UPB)  |  0.248 | [0.137, 0.358] | 미포함      |
| sig_포용 = (포용→OCB) − (포용→UPB) |  0.363 | [0.277, 0.444] | 미포함      |
| sig_형평 = (형평→UPB) − (형평→OCB) |  0.271 | [0.185, 0.360] | 미포함      |

**H4 판정: H4가 지지되었다**

## 3. 강건성 교차확인 (semopy vs. statsmodels HC3 OLS)

| Path     |   semopy β |   OLS(HC3) β |   OLS p |
|:---------|-----------:|-------------:|--------:|
| 포용 → OCB |      0.331 |        0.331 |  0      |
| 형평 → OCB |     -0.055 |       -0.055 |  0.0523 |
| 형평 → UPB |      0.216 |        0.216 |  0      |
| 포용 → UPB |     -0.032 |       -0.032 |  0.307  |
...
직접효과는 포용 → OCB β=0.331(95% BCa CI [0.269, 0.392]),
형평 → OCB β=-0.055(95% BCa CI [-0.109, 0.002]),
형평 → UPB β=0.216(95% BCa CI [0.151, 0.282]),
포용 → UPB β=-0.032(95% BCa CI [-0.095, 0.030])로 나타났다.
H4의 공식 검정을 위해 정의한 두 핵심 대비 중, d_OCB(포용→OCB − 형평→OCB)는
0.386(95% BCa CI [0.281, 0.486])로 0을
포함하지 않아, 포용적 조직문화가 형평 기반 조직문화보다
OCB에 대해 더 강한 직접효과를 갖는다는 예측이 지지되었다.
d_UPB(형평→UPB − 포용→UPB)는 0.248(95% BCa CI [0.137, 0.358])로
0을 포함하지 않아, 형평 기반 조직문화가 포용적 조직문화보다
UPB에 대해 더 강한 직접효과를 갖는다는 예측이 지지되었다.
종합하면, H4가 지지되었다. 보강 대비인 sig_포용(포용→OCB − 포용→UPB)과 sig_형평(형평→UPB − 형평→OCB)은
각각 0.363(95% BCa CI [0.277, 0.444],
0 미포함),
0.271(95% BCa CI [0.185, 0.360],
0 미포함)로 나타나,
